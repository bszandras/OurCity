...
void ResidentManager::calculateHappiness()
{
	// Vegigmegyunk az osszes lakon
	for (size_t i = 0; i < residents.size(); i++)
	{
		...
		if (residents[i].getHouse() != 0 && residents[i].getWorkplace() != 0)
		{
			Tile* house = world->getWrapper()->GetPointerToId(residents[i].getHouse());
			Tile* workplace = world->getWrapper()->GetPointerToId(residents[i].getWorkplace());
